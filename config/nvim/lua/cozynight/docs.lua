...
local Groups = require("cozynight.groups")
...
function M.extras()
  local Extra = require("cozynight.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/cozynight/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/cozynight/config.lua", "\n(--@class cozynight%.Config.-\n})")
